...
#define _BEG_ADDR(area_name_upper_case)   (GB_ ## area_name_upper_case ## _BEG_ADDR) 
...
#define IF_ADDR_IN_RANGE(area_name_upper_case, statement)                                   if (addr >= _BEG_ADDR(area_name_upper_case) && addr <= _END_ADDR(area_name_upper_case)) { statement }
...
#define DECL_MEM_ACCESSOR(access_type)                                                                                                                              \
    MEM_##access_type##_FUNC_DECL {                                                                                                                                 \
        ...
        MAKE_MEM_##access_type##_ACCESS_CALLBACK      (GB_SYSCLK_LSB_ADDR,          _GB_io_reg,             gb->io_regs)            /* Sysclk       -- FF03      */     \
        MAKE_MEM_##access_type##_RANGE_ACCESS_CALLBACK(TIMER_REGS,                  GB_timer,               gb->io_regs)            /* Timer        -- FF04-FF07 */     \
        MAKE_MEM_##access_type##_RANGE_ACCESS_CALLBACK(TIMER_INT_UNUSED_RANGE,      _GB_io_reg,             gb->io_regs)            /* Unused range -- FF08-FF0E */     \
        ...
        MAKE_MEM_##access_type##_RANGE_ACCESS_CALLBACK(APU_WAVEP_UNUSED_RANGE,      _GB_io_reg,             gb->io_regs)            /* Unused range -- FF27-FF29 */     \
...
BYTE GB_mem_read(GB_gameboy_t *gb, WORD addr) { 
    if (gb->mmu->is_dma_active) {
		...
		if (addr >= GB_OAM_BEG_ADDR && addr <= GB_OAM_END_ADDR) {
			return 0xFF;
        }
	}
...
void GB_dma_run(GB_gameboy_t *gb) {
	...
	switch (gb->mmu->dma_state) { 									
    	...
    	case DMA_RUNNING: {
			gb->mmu->addr_bus = GB_OAM_BEG_ADDR | gb->mmu->dma_offset;
